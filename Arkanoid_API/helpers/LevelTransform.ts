...
export const levelTransform = async (): Promise<void> => {
      ...
      const transformedData: Level[] = chunk
        ...
        ?.map((item: string[]) => {
          ...
          item.forEach((item: string) => {
            ...
            tempObj[String(splittedArrat[0])] = Number(
              splittedArrat[1]?.trim() ?? 0
            );
          });
  ...
  const deleteUselessData: Transform = new Transform({
    objectMode: true,
    transform(chunk: string, end: BufferEncoding, callback) {
  ...
  try {
    await pipeline(
      ...
      fs.createWriteStream(
        path.resolve("./Arkanoid_API/data/formattedLevels.txt")
      )
    );
  } catch (err) {
